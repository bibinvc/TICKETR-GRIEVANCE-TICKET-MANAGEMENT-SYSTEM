 ...
 */
@WebServlet("/auth/userticketdash")
public class TicketDashServ extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			//String query = "select * from ticket_tbl where assigndto='"+name+"' and status='Processing' ||  assigndto='"+name+"' and status='Hold' order by id desc";
			String query = "select * from ticket_tbl where assigndto='"+name+"' and status='Processing' || assigndto='"+name+"' and status='Hold' ||follower='"+name+"' and status='Processing' || follower='"+name+"' and status='Hold' ||requstuser='"+name+"' and status='Processing' || requstuser='"+name+"' and status='Hold' order by id desc";																			
			//select * from ticket_tbl where assigndto='"+name+"' and status='Processing' || assigndto='"+name+"' and status='Hold' ||follower='"+name+"' and status='Processing' || follower='"+name+"' and status='Hold' ||requstuser='"+name+"' and status='Processing' || requstuser='"+name+"' and status='Hold' order by id desc
			...
			request.getRequestDispatcher("/auth/userdashticket").forward(request, response);
